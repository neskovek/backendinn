...
export class ProjectController {
  ...
  @Get()
  @ApiOperation({ summary: 'Listar projetos (admin vê todos; herói vê apenas os seus)' })
  @ApiQuery({ name: 'status', required: false, enum: ProjectStatus })
  @ApiQuery({ name: 'userId', required: false, type: String, description: 'Filtrar por herói (apenas admin)' })
  ...
  @ApiResponse({ status: 401, description: 'Token JWT ausente ou inválido.' })
  @ApiResponse({ status: 403, description: 'Sem permissão para visualizar este projeto.' })
  ...
  @ApiResponse({ status: 401, description: 'Token JWT ausente ou inválido.' })
  @ApiResponse({ status: 403, description: 'Apenas administradores podem criar projetos.' })
  ...
  @ApiResponse({ status: 400, description: 'Dados inválidos.' })
  @ApiResponse({ status: 401, description: 'Token JWT ausente ou inválido.' })
  @ApiResponse({ status: 403, description: 'Apenas administradores podem editar projetos.' })
  @ApiResponse({ status: 404, description: 'Projeto não encontrado.' })
  ...
  @ApiResponse({ status: 401, description: 'Token JWT ausente ou inválido.' })
  @ApiResponse({ status: 403, description: 'Apenas administradores podem remover projetos.' })
  @ApiResponse({ status: 404, description: 'Projeto não encontrado.' })
